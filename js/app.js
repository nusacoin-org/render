...
const STYLE_DATA = {
  "Modern Minimalist": {
    ...
    materials: "matte painted walls, a polished concrete floor, and frosted glass partitions — window and door frames keep their original color and finish from the source model",
    ...
    vegetation_indoor: "a single, architectural indoor plant — a fiddle-leaf fig — standing in a concrete planter",
  ...
  "Industrial Loft": {
    ...
    vegetation_indoor: "one oversized fiddle-leaf fig in a raw steel planter, a soft green counterpoint to all that hard material",
  ...
  "Japandi Style": {
    ...
    vegetation_indoor: "a single bonsai beside a potted bamboo stalk",
...
// text happened to imply, which is why the same building came out with a
// different kusen color on every angle. This statement makes explicit that
// any material already visible/colored in the reference image is fixed
// ground truth, same as the geometry — the style only fills in gaps.
const MATERIAL_FIDELITY_LOCK =
  ...
  "window frames, door frames, roof covering, wall cladding, trim, structural elements (kusen included), and " +
  "the existing road/pavement/ground surface — must be reproduced exactly as shown, same hue, same finish, " +
  "same texture. If the reference image shows an asphalt street, it must stay a dark asphalt street; it must " +
  "never be replaced with concrete, pavers, or any other paving material. The design style described below " +
  "only governs surfaces and elements that are not already defined in the source model (general ambience, " +
  "staging, furnishing, and landscaping); it must never repaint, retint, resurface, or reinterpret a material " +
  "that already has a color or texture in the reference image.";
...
function buildPrompt({ type, style, track, lightingKey, outputKey }) {
  ...
  // 1. Geometry Lock Statement & Material Fidelity Lock
  ...
  // 5. Design Style & Material Details — furniture only makes sense (and is
  // only safe to request) for interior renders. Injecting living-room
  // furniture language into an exterior prompt was the main cause of a
  // sofa/interior appearing to "open up" through the facade.
  if (isExterior) {
  ...
  if (isExterior) {
    ...
    parts.push(`Within the property itself, ${styleData.ground_materials} ground the walkway and driveway — but any public road or street already visible in the reference image keeps its own existing surface and color untouched.`);
  } else {
